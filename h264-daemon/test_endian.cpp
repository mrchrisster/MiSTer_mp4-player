...
// Swaps adjacent bytes in a buffer to pre-compensate for FPGA endianness bug
void endian_swap_buffer(uint8_t* buf, int size) {
...
void test_endian_swap(volatile uint32_t* axi, uint8_t* yuv_y, uint8_t* yuv_u, uint8_t* yuv_v) {
    // 1. Write gradient to Y
    for (int i = 0; i < YUV_Y_SIZE; i++) {
        yuv_y[i] = i % 256;
    }
    
    // Write solid U/V (128 = neutral gray)
    memset(yuv_u, 128, YUV_U_SIZE);
    memset(yuv_v, 128, YUV_V_SIZE);

    // 2. Pre-swap the bytes!
    // The buggy FPGA will swap them back, so they should display correctly.
    endian_swap_buffer(yuv_y, YUV_Y_SIZE);
    endian_swap_buffer(yuv_u, YUV_U_SIZE);
    endian_swap_buffer(yuv_v, YUV_V_SIZE);
    ...
    printf("Pre-swapped gradient pattern written and DMA triggered.\n");
    printf("If the screen shows a smooth gradient, the endian bug is confirmed!\n");
}
...
int main(int argc, char** argv) {
    ...
    test_endian_swap(axi, yuv_y, yuv_u, yuv_v);
